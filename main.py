...
def main_worker(args):
    ...
    elif args.optim == "lamb_sam":
        optimizer = sam.LambSAM(
            model.parameters(),
            lr=args.lr,
            momentum=args.momentum,
            weight_decay=args.weight_decay,
    ...
    if args.rank == 0:
        ...
        os.makedirs(f"./trained_models/{args.arch}", exist_ok=True)
        ...
        model_path = "".join(
            [f"./trained_models/{args.arch}/model_", "_".join(save_list), ".ckpt"]
        )
        optim_path = "".join(
            [f"./trained_models/{args.arch}/optim_", "_".join(save_list), ".ckpt"]
        )
