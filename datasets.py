...
from timm.data.parsers import create_parser as create_parser_timm
...
class ImageNetTestDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if parser is None or isinstance(parser, str):
            parser = create_parser_timm(parser or "", root=root, class_map=class_map)
        self.parser = parser
...
def load_imagenet_tar(args):
    ...
    train_dataset = ImageDataset(
        root=imagenet_tar_traindir,
        parser=ParserImageInTar(imagenet_tar_traindir),
    ...
    test_dataset = ImageNetTestDataset(
        root=imagenet_tar_valdir,
        parser=ParserImageInTar(imagenet_tar_valdir),
        transform=test_transform,
        target_transform=val_arr,
    )
